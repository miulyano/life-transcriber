...
- Таймкоды в `.txt`-файлах транскрипции (YouTube-стиль): имя спикера отдельной строкой, под ним строки `[м:сс] текст` / `[ч:мм:сс] текст`; новый таймкод — на первой границе предложения после ~15 секунд от предыдущего. Короткие ответы в чате остаются без таймкодов

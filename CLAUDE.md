...
### Test-first порядок работы

Для новой фичи или багфикса:

1. Сначала написать тест на ожидаемое поведение (для бага — тест, воспроизводящий
   баг). Убедиться, что он падает на текущем коде.
2. Потом писать реализацию, пока тест не станет зелёным.
3. Перед `git push` — полный `pytest -v` зелёный (см. правило выше).

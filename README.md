...
- 🔗 **Ссылки на видео** — YouTube, RuTube, VK Video, Vimeo и [многие другие платформы](https://github.com/yt-dlp/yt-dlp/blob/master/supportedsites.md) (всё, что поддерживает `yt-dlp`). Ссылка распознаётся и в пересланных постах: в подписи к медиа и в скрытых гиперссылках (`text_link`)
...
- `MAX_CONCURRENT_TRANSCRIPTIONS=3` — сколько транскрибаций может идти одновременно; задачи сверх лимита ждут в очереди (статус «В очереди…»)
...
- Под транскрибацией, которая пришла `.txt`-файлом, нажми **«🧹 Очистить текст»** → получи полную очищенную версию в том же порядке и формате блоков, файл придёт с подписью `Очищенный текст: <Заголовок>` и строкой источника (`Источник: … · без таймкодов`)
...
- Во вкладке **«Мои записи»** Mini App — все сохранённые генерации с указанием источника (YouTube, Яндекс Диск, Голосовое, Файл…): пришли в чат (обычную или с таймкодами — если у записи сохранены сегменты) или удали; после отправки Mini App закрывается, прогресс и результат видны в чате; в присланном сообщении указаны источник и наличие таймкодов
...
Cookies применяются только к YouTube-ссылкам. Если файл отсутствует или
протух, бот пишет warning в лог и повторяет скачивание без cookies — обычные
публичные видео продолжают работать. Cookies живут недели–месяцы, после чего
их нужно переэкспортировать.
